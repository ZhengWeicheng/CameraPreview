...
Java_com_myopengl_zcweicheng_render_Renderer_nativeInit(JNIEnv *env, jclass, jobject jSurface,
...
//    ShaderBase shader = ShaderNV21();
    GLuint programYUV = createProgram(vertex_, fragment_);
//    delete shader;
    if (programYUV == 0) {
        return JNI_FALSE;
    }
    ...
    engineHolder->posDistance = (GLuint) glGetUniformLocation(programYUV, "distance");
    engineHolder->posNextFilterId = (GLuint) glGetUniformLocation(programYUV, "currentFilter");
    //tex
...
Java_com_myopengl_zcweicheng_render_Renderer_nativeRelease(JNIEnv *, jclass,jlong engine) {
    releaseGLContext((GLContextHolder *) engine);
}

JNIEXPORT void JNICALL
Java_com_myopengl_zcweicheng_render_Renderer_nativeRender(JNIEnv *env, jclass, jlong engine,jbyteArray data_,
                                                          jint frameWidth, jint frameHeight, jint degree,
                                                          jboolean mirror, jint outWidth, jint outHeight,
                                                          jfloat distance, jint nextFilterId, jlong facePtr) {
    ...
    renderFrame(engineHolder, data, frameWidth, frameHeight, degree, mirror, outWidth, outHeight,
                distance, nextFilterId, facePtr);

    env->ReleaseByteArrayElements(data_, data, JNI_ABORT);
}

JNIEXPORT void JNICALL Java_com_myopengl_zcweicheng_render_Renderer_nativeSwitchFilter(JNIEnv *env, jobject ,jlong engine,jobject mAssetManager, jint mFilterType,jstring vertex_,jstring frag_){
    GLContextHolder *engineHolder = (GLContextHolder *) engine;
...
void renderFrame(GLContextHolder *engineHolder, jbyte *data, jint width, jint height, jint degree,
    ...
    engineHolder->mDistance = distance;
    engineHolder->mNextFilterId = nextFilterId;

    glUniform1f(engineHolder->posDistance, engineHolder->mDistance);
    glUniform1f(engineHolder->posNextFilterId, engineHolder->mNextFilterId);
